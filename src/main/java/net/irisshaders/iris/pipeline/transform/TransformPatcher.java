...
// Vulkan Port: GLSL 460 version for SPIR-V compilation target
...
import net.irisshaders.iris.pipeline.transform.transformer.TerrainCoreTransformer;
import net.irisshaders.iris.pipeline.transform.transformer.TerrainTransformer;
...
public class TransformPatcher {
				...
				root.indexBuildSession(() -> {
					...
					if (Objects.requireNonNull(parameters.patch) == Patch.COMPUTE) {// we can assume the version is at least 400 because it's a compute shader
						...
						if (profile == Profile.CORE || version.number >= 150 && profile == null || isLine) {
							// Vulkan Port: patch version to 460 for SPIR-V compilation
							if (IrisLimits.VK_CONFORMANCE) {
								versionStatement.version = Version.fromNumber(460);
							} else if (version.number < 410) {
							...
							switch (parameters.patch) {
								case COMPOSITE:
									...
									break;
								case TERRAIN:
									TerrainParameters terrainParameters = (TerrainParameters) parameters;
									TerrainCoreTransformer.transform(transformer, tree, root, terrainParameters);
									break;
								case VANILLA:
							...
							switch (parameters.patch) {
								...
								case TERRAIN:
									TerrainParameters terrainParameters = (TerrainParameters) parameters;
									TerrainTransformer.transform(transformer, tree, root, terrainParameters);
									break;
								case VANILLA:
	...
	public static Map<PatchShaderType, String> patchTerrain(String name, String vertex, String geometry, String tessControl, String tessEval, String fragment,
														   AlphaTest alpha, ShaderAttributeInputs inputs,
														   Object2ObjectMap<Tri<String, TextureType, TextureStage>, String> textureMap) {
		return transform(name, vertex, geometry, tessControl, tessEval, fragment,
			new TerrainParameters(Patch.TERRAIN, textureMap, alpha, inputs));
	}
