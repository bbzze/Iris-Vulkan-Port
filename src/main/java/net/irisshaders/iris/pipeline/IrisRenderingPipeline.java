...
public class IrisRenderingPipeline implements WorldRenderingPipeline, ShaderRenderingPipeline, RenderTargetStateListener {
	// GL constants (inlined from GL15C/GL20C/GL21C/GL30C/GL43C)
	private static final int GL_TEXTURE0 = 0x84C0;
	private static final int GL_TEXTURE2 = 0x84C2;
	...
	private final VulkanTerrainPipeline terrainPipeline;
	...
	public IrisRenderingPipeline(ProgramSet programSet) {
		...
		if (!programSet.getPackDirectives().getBufferObjects().isEmpty()) {
			...
			for (int i = 0; i < Math.min(16, SamplerLimits.get().getMaxShaderStorageUnits()); i++) {
				IrisRenderSystem.bindBufferBase(GL_SHADER_STORAGE_BUFFER, i, 0);
			}
		}
		...
		IrisRenderSystem.activeTexture(GL_TEXTURE2);
		...
		IrisRenderSystem.activeTexture(GL_TEXTURE0);
		...
		IntFunction<ProgramSamplers> createTerrainSamplers = (programId) -> {
			...
			if (IrisSamplers.hasShadowSamplers(customTextureSamplerInterceptor)) {
				// we compiled the non-terrain version of this program first... so if this is somehow null, something
		...
		IntFunction<ProgramSamplers> createShadowTerrainSamplers = (programId) -> {
			...
			if (IrisSamplers.hasShadowSamplers(customTextureSamplerInterceptor)) {
				// We don't compile terrain shadow programs unless there's a shadow pass... And a shadow pass
		...
		// TODO: Create fallback terrain shaders if the pack doesn't provide terrain shaders
		this.terrainPipeline = new VulkanTerrainPipeline(this, resolver, programSet, createTerrainSamplers,
		...
		// Patch terrain shaders (transform shader pack sources for VulkanMod vertex format)
		this.terrainPipeline.patchShaders();

		// Activate terrain pipeline hook — compiles Iris terrain GLSL to Vulkan GraphicsPipelines
		// and hooks into PipelineManager.setShaderGetter() so terrain draws use Iris shaders
		net.irisshaders.iris.pipeline.terrain.IrisTerrainRenderHook.getInstance().activate(this.terrainPipeline);
	...
	public int getNoiseTextureId() {
		if (customTextureManager != null && customTextureManager.getNoiseTexture() != null) {
			return customTextureManager.getNoiseTexture().getTextureId().getAsInt();
		}
		return 0;
	}
	...
	public void onSetShaderTexture(int id) {
		if (shouldBindPBR && isRenderingWorld) {
			...
			if (textureFormat != null) {
				int previousBinding = IrisRenderSystem.getBoundTextureId(IrisRenderSystem.getActiveTexture());
				...
				IrisRenderSystem.bindTexture(previousBinding);
			}
	...
	public void beginLevelRendering() {
		...
		for (GlImage image : clearImages) {
			IrisRenderSystem.clearTexImage(image.getId(), 0, image.getFormat().getGlFormat(), image.getPixelType().getGlFormat(), null);
		}
	...
	public void destroy() {
		...
		// Deactivate terrain pipeline hook before destroying shaders
		net.irisshaders.iris.pipeline.terrain.IrisTerrainRenderHook.getInstance().deactivate();
		...
		for (int i = 0; i < 16; i++) {
			IrisRenderSystem.activeTexture(GL_TEXTURE0 + i);
			IrisRenderSystem.unbindAllSamplers();
			IrisRenderSystem.bindTexture(0);
		}
	...
	@Override
	public VulkanTerrainPipeline getTerrainPipeline() {
