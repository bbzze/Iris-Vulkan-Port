...
/**
 * Texture format interface - Vulkan Port.
 *
 * GL11 constants inlined as hex values.
 */
public interface TextureFormat {
	// GL texture constants (inlined from GL11)
	...
	default void setupTextureParameters(PBRType pbrType, AbstractTexture texture) {
		if (!canInterpolateValues(pbrType)) {
			int minFilter = IrisRenderSystem.getTexParameteri(texture.getId(), GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER);
			...
			IrisRenderSystem.texParameteri(texture.getId(), GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, mipmap ? GL_NEAREST_MIPMAP_NEAREST : GL_NEAREST);
			IrisRenderSystem.texParameteri(texture.getId(), GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
		}
	}
