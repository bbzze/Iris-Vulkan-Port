...
public class Iris {
	public static final String MODID = "iris-vulkan";
	...
	public static void setDebug(boolean enable) {
		...
		if (enable) {
			...
			// Skip GlDebug.enableDebugCallback() - it requires OpenGL which is not available under VulkanMod
			success = 1;
		}
	...
	private static WorldRenderingPipeline createPipeline(NamespacedId dimensionId) {
		...
		try {
			IrisRenderingPipeline pipeline = new IrisRenderingPipeline(programs);
			// Restore VulkanMod's render state after pipeline init
			// Pipeline creation creates GL framebuffers which disrupts VulkanMod's active render pass
