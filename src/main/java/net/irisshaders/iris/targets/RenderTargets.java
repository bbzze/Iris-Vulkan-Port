...
/**
 * Render target manager - Vulkan Port.
 *
 * Manages the array of color render targets + depth textures used by Iris's
 * shader pipeline. GL constants inlined to remove LWJGL OpenGL dependency.
 */
public class RenderTargets {
	...
	private void create(int index) {
		...
		net.irisshaders.iris.Iris.logger.info("[RenderTargets] Creating colortex{}: format={} (0x{}) dimensions={}x{}",
			index, settings.getInternalFormat().name(), Integer.toHexString(settings.getInternalFormat().getGlFormat()),
		...
		net.irisshaders.iris.Iris.logger.info("[RenderTargets] colortex{} texIDs: main={} alt={} ({}x{})",
			index, targets[index].getMainTexture(), targets[index].getAltTexture(),
			targets[index].getWidth(), targets[index].getHeight());
	}
	...
	public void copyPreTranslucentDepth() {
		// Vulkan: always use explicit depth copy (copyStrategy uses GL-style framebuffer binding)
		IrisRenderSystem.copyDepthImage(currentDepthTexture, noTranslucents.getTextureId(), cachedWidth, cachedHeight);
		translucentDepthDirty = false;
	}

	public void copyPreHandDepth() {
		// Vulkan: always use explicit depth copy
		IrisRenderSystem.copyDepthImage(currentDepthTexture, noHand.getTextureId(), cachedWidth, cachedHeight);
