...
public class IrisLodRenderProgram {
	...
	private IrisLodRenderProgram(String name, boolean isShadowPass, boolean translucent, BlendModeOverride override, BufferBlendOverride[] bufferBlendOverrides, String vertex, String tessControl, String tessEval, String geometry, String fragment, CustomUniforms customUniforms, IrisRenderingPipeline pipeline) {
		id = IrisRenderSystem.glCreateProgram();

		IrisRenderSystem.glBindAttribLocation(this.id, 0, "vPosition");
		IrisRenderSystem.glBindAttribLocation(this.id, 1, "iris_color");
		IrisRenderSystem.glBindAttribLocation(this.id, 2, "irisExtra");

		this.bufferBlendOverrides = bufferBlendOverrides;

		GlShader vert = new GlShader(ShaderType.VERTEX, name + ".vsh", vertex);
		IrisRenderSystem.glAttachShader(id, vert.getGlHandle());

		GlShader tessCont = null;
		if (tessControl != null) {
			tessCont = new GlShader(ShaderType.TESSELATION_CONTROL, name + ".tcs", tessControl);
			IrisRenderSystem.glAttachShader(id, tessCont.getGlHandle());
		}

		GlShader tessE = null;
		if (tessEval != null) {
			tessE = new GlShader(ShaderType.TESSELATION_EVAL, name + ".tes", tessEval);
			IrisRenderSystem.glAttachShader(id, tessE.getGlHandle());
		}

		GlShader geom = null;
		if (geometry != null) {
			geom = new GlShader(ShaderType.GEOMETRY, name + ".gsh", geometry);
			IrisRenderSystem.glAttachShader(id, geom.getGlHandle());
		}

		GlShader frag = new GlShader(ShaderType.FRAGMENT, name + ".fsh", fragment);
		IrisRenderSystem.glAttachShader(id, frag.getGlHandle());

		IrisRenderSystem.glLinkProgram(this.id);
		int status = IrisRenderSystem.glGetProgrami(this.id, 35714);
		if (status != 1) {
			String message = "Shader link error in Iris DH program! Details: " + IrisRenderSystem.glGetProgramInfoLog(this.id);
			this.free();
			throw new RuntimeException(message);
		} else {
			IrisRenderSystem.useProgram(this.id);
		}
	...
	public int tryGetUniformLocation2(CharSequence name) {
		return IrisRenderSystem.glGetUniformLocation(this.id, name);
	}
	...
	public void bind() {
		IrisRenderSystem.useProgram(id);
	...
	public void free() {
		IrisRenderSystem.glDeleteProgram(id);
	}
	...
	private void setUniform(int index, float value) {
		IrisRenderSystem.glUniform1f(index, value);
	}
	...
	private void setUniform(int index, Vec3f pos) {
		IrisRenderSystem.glUniform3f(index, pos.x, pos.y, pos.z);
	}
