...
/**
 * Shadow render targets - Vulkan Port.
 *
 * Manages shadow map render targets (depth + optional color).
...
public class ShadowRenderTargets {
	...
	// Shadow terrain rendering framebuffer — includes all color attachments
	// that the shadow shader writes to (e.g. shadowcolor0 + shadowcolor1).
	// depthSourceFb only has drawBuffer [0], which discards writes to location 1.
	private GlFramebuffer shadowTerrainRenderFb;
	...
	/**
	 * Returns a framebuffer suitable for terrain shadow rendering, with both
	 * shadowcolor0 and shadowcolor1 as color attachments. This is needed because
	 * shadow shaders output to both location 0 and location 1.
	 * The depthSourceFb only has drawBuffer [0], so writes to location 1 are discarded.
	 */
	public GlFramebuffer getShadowTerrainRenderFramebuffer() {
		if (shadowTerrainRenderFb == null) {
			// Create with drawBuffers [0, 1] and depth attachment
			shadowTerrainRenderFb = createShadowFramebuffer(
				com.google.common.collect.ImmutableSet.of(), new int[]{0, 1});
			net.irisshaders.iris.Iris.logger.info(
				"[ShadowRenderTargets] Created shadow terrain render FB with drawBuffers [0,1]: " +
				"shadowcolor0={} shadowcolor1={} depth={}",
				getOrCreate(0).getMainTexture(), getOrCreate(1).getMainTexture(),
				mainDepth.getTextureId());
		}
		return shadowTerrainRenderFb;
	}

	public void copyPreTranslucentDepth() {
		// Vulkan: always use explicit depth copy (blitFramebuffer/DepthCopyStrategy use GL-style ops)
		IrisRenderSystem.copyDepthImage(mainDepth.getTextureId(), noTranslucents.getTextureId(), resolution, resolution);
