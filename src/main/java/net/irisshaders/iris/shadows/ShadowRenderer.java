...
/**
 * Shadow renderer - Vulkan Port.
 *
 * ARBTextureSwizzle/GL20C/GL30C constants inlined as hex values.
 */
public class ShadowRenderer {
	...
	private static final int GL_TEXTURE4 = 0x84C4;
	...
	private static int shadowTerrainLogCount = 0;
	...
	private void configureDepthSampler(int glTextureId, PackShadowDirectives.DepthSamplingSettings settings) {
		if (settings.getHardwareFiltering() && !separateHardwareSamplers) {
			...
			IrisRenderSystem.texParameteri(glTextureId, GL_TEXTURE_2D, GL_TEXTURE_COMPARE_MODE, GL_COMPARE_REF_TO_TEXTURE);
		}
		...
		IrisRenderSystem.texParameteriv(glTextureId, GL_TEXTURE_2D, GL_TEXTURE_SWIZZLE_RGBA,
	...
	private void configureSampler(int glTextureId, PackShadowDirectives.SamplingSettings settings) {
		if (settings.getMipmap()) {
			int filteringMode = settings.getNearest() ? GL_NEAREST_MIPMAP_NEAREST : GL_LINEAR_MIPMAP_LINEAR;
			mipmapPasses.add(new MipmapPass(glTextureId, filteringMode));
		}

		if (!settings.getNearest()) {
			// Make sure that things are smoothed
			IrisRenderSystem.texParameteri(glTextureId, GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
			IrisRenderSystem.texParameteri(glTextureId, GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
		} else {
			IrisRenderSystem.texParameteri(glTextureId, GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
			IrisRenderSystem.texParameteri(glTextureId, GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
		}
	}
	...
	private void setupMipmappingForTexture(int texture, int filteringMode) {
		...
		IrisRenderSystem.texParameteri(texture, GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, filteringMode);
	}
	...
	public void renderShadows(LevelRendererAccessor levelRenderer, Camera playerCamera) {
		...
		ACTIVE = true;
		net.vulkanmod.render.chunk.WorldRenderer.setShadowPassActive(true);
		...
		if (shouldRenderTerrain) {
			if (shadowTerrainLogCount < 3) {
				net.irisshaders.iris.Iris.logger.info("[ShadowRenderer] Rendering shadow terrain: resolution={} hookActive={} hasFB={}",
					resolution, terrainHook.isActive(), targets.getDepthSourceFb() != null);
				shadowTerrainLogCount++;
			}
		...
		// Reset our viewport in case VulkanMod overrode it
		RenderSystem.viewport(0, 0, resolution, resolution);
		...
		// Clear the shadow framebuffer override so normal gbuffer rendering resumes
		if (terrainHook.isActive()) {
			terrainHook.setShadowFramebuffer(null);
		}
		...
		ACTIVE = false;
		net.vulkanmod.render.chunk.WorldRenderer.setShadowPassActive(false);
