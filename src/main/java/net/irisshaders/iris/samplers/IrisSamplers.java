...
public class IrisSamplers {
	...
	private static int shadowColorDiagCount = 0;
		...
		for (int i = startIndex; i < renderTargets.getRenderTargetCount(); i++) {
			...
			IntSupplier texture = () -> {
				...
				if (flippedBuffers.contains(index)) {
					texId = target.getAltTexture();
				} else {
					texId = target.getMainTexture();
				}
				return texId;
			};
	...
	public static boolean addShadowSamplers(SamplerHolder samplers, ShadowRenderTargets shadowRenderTargets, ImmutableSet<Integer> flipped, boolean separateHardwareSamplers) {
		...
		// Diagnostic: log shadow color texture IDs (once)
		if (shadowColorDiagCount < 3) {
			shadowColorDiagCount++;
			for (int d = 0; d < Math.min(2, shadowRenderTargets.getRenderTargetCount()); d++) {
				RenderTarget rt = shadowRenderTargets.get(d);
				if (rt != null) {
					net.irisshaders.iris.Iris.logger.info("[SHADOW_SAMPLER] shadowcolor{}: mainTex={} altTex={} ({}x{})",
						d, rt.getMainTexture(), rt.getAltTexture(), rt.getWidth(), rt.getHeight());
				} else {
					net.irisshaders.iris.Iris.logger.warn("[SHADOW_SAMPLER] shadowcolor{}: NOT CREATED (null)", d);
				}
			}
		}
